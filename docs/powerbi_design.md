...
This document is the **visual source of truth** for the five-page CHRO report.

Build it by hand in Power BI Desktop following the companion artefacts:

| File | Purpose |
|---|---|
| `powerbi/validate_gold_for_powerbi.py` | Run **first** — verifies the Parquet contract |
| `powerbi/AgileHRTheme.json` | Load via *View → Themes → Browse for themes* |
| `powerbi/power_query_import_guide.md` | Step-by-step data import + relationships |
| `powerbi/dax_measures.md` | Complete `_Measures` library (paste verbatim) |
| `powerbi/page_build_checklist.md` | Tick-as-you-go execution checklist |
| **this file** | Visual design spec for each page |

Save the finished report as `powerbi/AgileHRCopilot.pbix`, then export each page to PNG into `docs/images/`.
...
Every measure used by these pages is defined verbatim in:

> `powerbi/dax_measures.md`

Paste them into a `_Measures` table (see the companion file's section 8 for the recommended display folders). The pages below reference the **canonical measure names** from that library:

`Headcount`, `Headcount (k-safe)`, `Average Headcount`, `Attrition Count`, `Attrition Rate`, `Attrition Rate YoY`, `Attrition Rate YoY (pp delta)`, `Attrition Rate (k-safe)`, `High Risk Count`, `Medium Risk Count`, `Low Risk Count`, `Average Risk Score`, `Estimated Cost of Attrition`, `Avg Pulse Score`, `Engagement Index`, `Response Rate`, `Avg Job Satisfaction`, `Avg Work-Life Balance`, `Gender Ratio F`, `Diversity Index`, `Open Requisitions`, `Total Applications`, `Hires`, `Hire Rate`, `Avg Time to Hire`, `Offer Acceptance Rate`, `Funnel Conversion Applied to Hired`, `Promotion Proxy Rate`.
...
| Attrition rate | `[Attrition Rate]` (%) | `[Attrition Rate YoY (pp delta)]` shown sign-aware in green / red |
| High-risk count | `[High Risk Count]` | "% of workforce" sub-line |
| Engagement index | `[Engagement Index]` | "/ 100 · last pulse" |
| Open requisitions | `[Open Requisitions]` | `[Avg Time to Hire]` days sub-line |
...
| YoY change | `[Attrition Rate YoY (pp delta)]` formatted with `+`/`-` and red/green |
| High-risk count | `[High Risk Count]` |
| Avg risk score | `[Average Risk Score]` (0–1, format as %) |
...
2. **Composition AgeBand × Dept** — heatmap matrix using `[Headcount (k-safe)]`; cells display blank/`–` when the cohort has fewer than 25 members.
